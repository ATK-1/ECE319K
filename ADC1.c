/* ADC1.c
 * Ashton Knecht and Luis Aceves-Iniguez
 * Modified: 3/30/2024
...
uint32_t ADCin(void){
  ...
  // return digital result (0 to 4095)
      ...
      uint32_t volatile delaytime = ADC1->ULLMEM.STATUS;    // 3) Time to Let ADC Start
      while((ADC1->ULLMEM.STATUS & 0x01)== 0x01){}          // 4) Wait for Completion
      return ADC1->ULLMEM.MEMRES[0];                        // 5) Returning 12-bit Result
}
